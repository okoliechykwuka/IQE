    ...
    def evaluate_sliding(self, model_name):
        ...
        for chunk in self.sliding_window(self.content):
            content_eval_tmpl_str = LLamaPromptTemplate(
                self.sliding_prompt
            ).partial_format(content=chunk, N=self.N, previous_summary=context_summary)
            self.app.update_prompts(
                {
                    "response_synthesizer:refine_template": content_eval_tmpl_str,
                    "response_synthesizer:text_qa_template": content_eval_tmpl_str,
                }
            )
...
class DesignEvaluator(Evaluator):
    ...
    def eval_design(self, slide=False):
        if slide:
            ...
            content_eval_tmpl_str = LLamaPromptTemplate(
                self.base_prompt
            ).partial_format(content=self.content, N=self.N)
            self.app.update_prompts(
                {
                    "response_synthesizer:refine_template": content_eval_tmpl_str,
                    "response_synthesizer:text_qa_template": content_eval_tmpl_str,
